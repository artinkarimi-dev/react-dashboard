...
const SummaryCard = ({ title, value, Icon, color }) => {
  ...
  return (
    <div className="group relative bg-white p-7 rounded-[2rem] border border-gray-100 shadow-[0_20px_50px_rgba(0,0,0,0.05)] transition-all duration-500 hover:shadow-[0_30px_70px_rgba(0,0,0,0.08)] hover:-translate-y-2 overflow-hidden">
      
      <div
        className={`absolute -right-10 -top-10 w-40 h-40 rounded-full blur-[80px] opacity-10 group-hover:opacity-20 transition-opacity duration-500 bg-gradient-to-br ${theme.glow}`}
      />
      ...
      <div className="relative z-10 flex flex-col h-full justify-between">
        ...
        <div className="flex justify-between items-start mb-8">
          <div className="relative">
            ...
            />
            <div
              className={`relative w-14 h-14 rounded-2xl flex items-center justify-center ${theme.iconBox} ring-1 shadow-sm transition-transform duration-500 group-hover:scale-110 group-hover:rotate-6`}
            >
        ...
        <div className="space-y-2">
          <h4 className="text-xs font-bold text-gray-400 uppercase tracking-[0.18em]">
            {title}
          </h4>
          ...
          <div className="flex items-end justify-between gap-3">
            ...
            <svg
              className="w-16 h-8 text-gray-200 group-hover:text-slate-300 transition-colors duration-500"
              viewBox="0 0 100 40"
              fill="none"
            >
              <path
